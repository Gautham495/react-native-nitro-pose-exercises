...
const FEET_LANDMARKS = new Set([27, 28, 29, 30, 31, 32]);
const FEET_VIS_THRESHOLD = 0.5;
...
export function NormalCameraView({
  ...
  const picture = useDerivedValue(() => {
    ...
    for (let i = 0; i < SKELETON_CONNECTIONS.length; i++) {
      ...
      const pb = lms[b];
      if (!pa || !pb) continue;
      const aThresh = FEET_LANDMARKS.has(a)
        ? FEET_VIS_THRESHOLD
        : VIS_THRESHOLD;
      const bThresh = FEET_LANDMARKS.has(b)
        ? FEET_VIS_THRESHOLD
        : VIS_THRESHOLD;
      if (pa.visibility < aThresh || pb.visibility < bThresh) continue;
    ...
    for (let idx = 11; idx < lms.length; idx++) {
      const lm = lms[idx];
      if (!lm) continue;
      const thresh = FEET_LANDMARKS.has(idx)
        ? FEET_VIS_THRESHOLD
        : VIS_THRESHOLD;
      if (lm.visibility < thresh) continue;
