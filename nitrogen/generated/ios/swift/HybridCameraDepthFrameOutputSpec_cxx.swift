...
open class HybridCameraDepthFrameOutputSpec_cxx : HybridCameraOutputSpec_cxx {
   ...
   */
  private var __cxxPart: bridge.std__weak_ptr_margelo__nitro__camera__HybridCameraDepthFrameOutputSpec_
   ...
   * The C++ part is a `std::shared_ptr<margelo::nitro::camera::HybridCameraDepthFrameOutputSpec>`.
   */
  public func getCxxPart() -> bridge.std__shared_ptr_margelo__nitro__camera__HybridCameraDepthFrameOutputSpec_ {
    let cachedCxxPart = self.__cxxPart.lock()
    if Bool(fromCxx: cachedCxxPart) {
      return cachedCxxPart
    } else {
      let newCxxPart = bridge.create_std__shared_ptr_margelo__nitro__camera__HybridCameraDepthFrameOutputSpec_(self.toUnsafe())
      __cxxPart = bridge.weakify_std__shared_ptr_margelo__nitro__camera__HybridCameraDepthFrameOutputSpec_(newCxxPart)
      return newCxxPart
    }
  }

  public override func getCxxPart() -> bridge.std__shared_ptr_margelo__nitro__camera__HybridCameraOutputSpec_ {
    let ownCxxPart: bridge.std__shared_ptr_margelo__nitro__camera__HybridCameraDepthFrameOutputSpec_ = getCxxPart()
  ...
  // Methods
  @inline(__always)
  public final func setOnDepthFrameCallback(onDepthFrame: bridge.std__optional_std__function_bool_const_std__shared_ptr_margelo__nitro__camera__HybridDepthSpec______depth______) -> bridge.Result_void_ {
    do {
      try self.__implementation.setOnDepthFrameCallback(onDepthFrame: { () -> ((_ depth: (any HybridDepthSpec)) -> Bool)? in
        if bridge.has_value_std__optional_std__function_bool_const_std__shared_ptr_margelo__nitro__camera__HybridDepthSpec______depth______(onDepthFrame) {
          let __unwrapped = bridge.get_std__optional_std__function_bool_const_std__shared_ptr_margelo__nitro__camera__HybridDepthSpec______depth______(onDepthFrame)
          return { () -> ((any HybridDepthSpec)) -> Bool in
            let __wrappedFunction = bridge.wrap_Func_bool_std__shared_ptr_margelo__nitro__camera__HybridDepthSpec_(__unwrapped)
            return { (__depth: (any HybridDepthSpec)) -> Bool in
              let __result = __wrappedFunction.call({ () -> bridge.std__shared_ptr_margelo__nitro__camera__HybridDepthSpec_ in
